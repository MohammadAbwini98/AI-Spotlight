<#
.SYNOPSIS
  Reconstructs the original AI Spotlight GGUF model file from its verified
  parts. Recipient-side script: requires only PowerShell.

.DESCRIPTION
  Reads manifest.json in this directory, checks destination disk space,
  verifies every part in .\parts (existence, count, ordering, size,
  SHA-256), concatenates the parts into a temporary .partial file with
  binary streams, and renames it to the final .gguf name only after size
  and SHA-256 match the manifest. Never imports or launches the model.

  Streaming I/O with a bounded buffer is used throughout; memory usage stays
  constant regardless of model size (100 MB, 7.38 GB, or larger).
#>
[CmdletBinding()]
param(
  [switch]$Force
...
# Required free space = model bytes plus the larger of 256 MB or 5%.
$SpaceMarginBytes = 256MB
$SpaceMarginRatio = 0.05
...
function FormatGigabytes([long]$bytes) {
...
if ($manifest.packageType -and $manifest.packageType -ne 'ai-model') {
  Fail ("ERROR: wrong package type for this script: '$($manifest.packageType)'. " +
    'Use Reassemble.ps1 for application packages.')
}
$modelFile = $manifest.modelFile
if (-not $modelFile) { $modelFile = $manifest.originalFile }
$requiredText = @('application', 'originalSha256')
...
if (-not $modelFile -or -not ($modelFile -is [string]) -or [string]::IsNullOrWhiteSpace($modelFile)) {
  Fail "ERROR: invalid manifest: 'modelFile' is missing or empty."
}
...
if ($modelFile -match '[\\/]' -or $modelFile -match '\.\.') {
  Fail 'ERROR: invalid manifest: modelFile must be a plain file name.'
}
...
$destination = Join-Path $root $modelFile
...
# --- Disk-space preflight (before any expensive verification) ------------------
$margin = [Math]::Max([long]$SpaceMarginBytes, [long]($originalSize * $SpaceMarginRatio))
[long]$requiredBytes = $originalSize + $margin
try {
  ...
  Fail "ERROR: could not determine free disk space for:`n$root"
}
if ($availableBytes -lt $requiredBytes) {
  Fail ("ERROR: Insufficient free disk space.`n`nRequired:`n{0} ({1} bytes)`n`nAvailable:`n{2} ({3} bytes)`n`nModel reconstruction was not started." -f
    (FormatGigabytes $requiredBytes), $requiredBytes,
    (FormatGigabytes $availableBytes), $availableBytes)
}
...
for ($i = 1; $i -le $partCount; $i++) {
  $expectedNames += "$modelFile.part" + $i.ToString("D$width")
}
$seenNames = @{}
$orderedNames = @()
...
# A stale partial from an interrupted run is never a valid model: remove it now
# so this run starts clean. The final file is only touched after verification.
if (Test-Path -LiteralPath $partialPath) {
  Remove-Item -LiteralPath $partialPath -Force
}

# --- Per-part size + hash verification -----------------------------------------
Write-Output 'Verifying parts...'
$verified = 0
for ($i = 0; $i -lt $partCount; $i++) {
  ...
  if ($actualHash -ne $entry.sha256.ToLowerInvariant()) {
    Fail ("ERROR: SHA-256 mismatch for part:`n{0}`nExpected:`n{1}`nActual:`n{2}`n`nThe reconstructed model has NOT been accepted." -f
      $entry.name, $entry.sha256, $actualHash)
  }
  $verified++
  Write-Output ("Part {0} / {1}" -f ($i + 1), $partCount)
}

# --- Reassembly into .partial (binary streams, bounded buffer) ------------------
Write-Output 'Reassembling model...'
...
$outStream = $null
try {
  $outStream = [System.IO.File]::Create($partialPath)
  ...
  foreach ($entry in $manifest.parts) {
    ...
    $percent = [Math]::Floor(($writtenTotal * 100.0) / $originalSize)
    Write-Output ("{0} / {1}`n{2}%" -f (FormatGigabytes $writtenTotal), (FormatGigabytes $originalSize), $percent)
  }
}
finally {
...
# --- Final acceptance: size + hash, then atomic rename --------------------------
$partialSize = (Get-Item -LiteralPath $partialPath).Length
if ($partialSize -ne $originalSize) {
  Remove-Item -LiteralPath $partialPath -Force -ErrorAction SilentlyContinue
  Fail ("ERROR: reconstructed size mismatch:`nExpected: {0}`nActual: {1}`n`nThe reconstructed model has NOT been accepted." -f
    $originalSize, $partialSize)
}
Write-Output 'Verifying final SHA-256...'
$partialHash = GetFileSha256 $partialPath
if ($partialHash -ne $manifest.originalSha256.ToLowerInvariant()) {
  Remove-Item -LiteralPath $partialPath -Force -ErrorAction SilentlyContinue
  Fail ("ERROR: SHA-256 mismatch:`n`nExpected:`n{0}`n`nActual:`n{1}`n`nThe reconstructed model has NOT been accepted." -f
    $manifest.originalSha256, $partialHash)
}
Move-Item -LiteralPath $partialPath -Destination $destination -Force

Write-Output ''
Write-Output 'AI Spotlight model package reconstructed successfully.'
Write-Output ''
Write-Output 'Model:'
Write-Output $modelFile
Write-Output ''
Write-Output 'Parts:'
Write-Output "$verified / $partCount verified"
Write-Output ''
Write-Output 'MODEL SHA-256: VERIFIED'
Write-Output ''
Write-Output 'The reconstructed model is identical to the distributed GGUF.'
exit 0
