...
- `npm test`: 170 tests across 33 files (+1 env-gated real-model file): all previous coverage plus share-package split/reassembly (bit-exact reconstruction across chunk boundaries, missing/corrupt/tampered-manifest/existing-destination rejection, 4-digit padding past 999 parts, `verifySharePackage` manifest/part/sum/original checks, `release:split` wiring) alongside native-dependency validation (synthetic PE32/PE32+ import + delay-load parsing, bundled/native/declared classification, missing-file/hash/arch failures, committed-manifest consistency, release-pipeline wiring), the AI generation-phase transitions (preparing/thinking/responding, failure/cancel clearing, hermetic manifest + injected fakes), and Thinking/Responding pill markup with aria-live and Stop affordance.
- `node scripts/release-integrity.cjs --verify-share <share-dir> [--original <signed-exe>]`: verifies a split share package (manifest structure, exact part set, per-part size/SHA-256, size totals, and signed-artifact hash correspondence). Also runs automatically per package inside `npm run release:split`.
