...
- `npm test`: 183 tests across 34 files (+1 env-gated real-model file): all previous coverage plus model share packaging (simulated-GGUF round-trip with `ai-model` manifest shape, manifest-derived metadata, GGUF pre-validation rejection, missing/corrupt/tampered/extra-part/existing-output/disk-space negatives, `verifySharePackage` model cases, gitignore + command wiring) alongside application split/reassembly (bit-exact reconstruction across chunk boundaries, missing/corrupt/tampered-manifest/existing-destination rejection, 4-digit padding past 999 parts, `verifySharePackage` manifest/part/sum/original checks, `release:split` wiring), native-dependency validation (synthetic PE32/PE32+ import + delay-load parsing, bundled/native/declared classification, missing-file/hash/arch failures, committed-manifest consistency, release-pipeline wiring), the AI generation-phase transitions (preparing/thinking/responding, failure/cancel clearing, hermetic manifest + injected fakes), and Thinking/Responding pill markup with aria-live and Stop affordance.
- `node scripts/release-integrity.cjs --verify-share <share-dir> [--original <file>]`: verifies a split share package, understanding `packageType` `application` (default) and `ai-model` (manifest structure, exact part set, per-part size/SHA-256, size totals, and source-file hash correspondence with `MODEL/APPLICATION SHARE PACKAGE: PASS/FAIL` diagnostics). Streaming SHA-256 throughout, so multi-GB models verify in constant memory. Also runs automatically per package inside `npm run release:split` and the complete offline flow.
