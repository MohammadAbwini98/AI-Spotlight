...
- Vitest regression suite passes with 183 tests across 34 files (+1 env-gated real-model file).
...
- DeepDive 1.0.2 is the signed private release containing the Thinking/Responding UX and the app-local VC++ runtime; its Portable and Setup artifacts passed typecheck, lint, 157 tests, build, native-dependency validation (53 binaries), signature, migration, packaged-native-dependency, and SHA-256 (255 files) verification, plus a packaged-server real-model smoke (health ok, HTTP 200 inference, app-local module paths, zero orphans). Both executables additionally ship as verified 20 MB split share packages (`release/share/1.0.2/`, 7 parts each): recipient-side reassembly reproduces bit-identical, signature-Valid executables, and the reassembled Portable launches (CDP alive), runs its bundled llama runtime, and exits 0 with zero orphans. The 7.38 GB Gemma model ships as a separate verified split package (71 x 100 MB parts, `packageType: ai-model` manifest, `Reassemble-Model.ps1` with disk preflight/progress/`.partial` safety): recipient-side reassembly reproduces the bit-identical GGUF, and the reconstructed file passes production import validation, loads in llama-server, and answers a real prompt. The complete offline set (`AI-Spotlight-1.0.2/` with `Application/{Setup,Portable}` + `Model/`) is produced by one `release:share:offline` command. Clean-machine, physical-offline, and post-reboot cold-start runs remain user steps.
