...
- An optional split-share distribution utility (`npm run release:split`) that divides each final signed executable into deterministic zero-padded parts with per-part SHA-256, a structural manifest, a PowerShell-only `Reassemble.ps1` (verifies parts, ordering, sizes, hashes, and final identity; never executes the output), and a recipient README — without touching the signed artifacts.
- An optional separate model distribution (`npm run release:split:model`, or the complete `AI-Spotlight-<version>/` offline set via `npm run release:share:offline`) that splits the GGUF independently (default 100 MB chunks, GGUF pre-validation, `ai-model` manifest with locally computed hashes only) with a PowerShell-only `Reassemble-Model.ps1` (disk-space preflight, progress, `.partial` interruption safety, final SHA-256 gate; never imports or launches). Application and model packages transfer, verify, and evolve independently; the GGUF is never embedded in installers, resources, or the repository.
