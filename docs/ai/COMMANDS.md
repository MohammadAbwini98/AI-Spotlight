...
| `npm run release:split:model` | Optional distribution-only step: split a GGUF model into a verified share package (default 100 MB chunks) with `packageType: ai-model` manifest, `Reassemble-Model.ps1`, and model README; validates GGUF magic/extension/size first |
| `npm run release:share:offline` | Optional distribution-only step: build the complete offline set (`AI-Spotlight-<version>/` with `Application/{Setup,Portable}`, `Model/<gguf>/`, root README) via `Create-SharePackage.ps1 -ModelPath <gguf>`; app/model chunk sizes independently configurable |
